...
export default function ListeOmraPage() {
  ...
  const openModal = (omra = null) => {
    ...
    if (omra) {
      form.setFieldsValue({
        ...omra,
        dateDepart: omra.dateDepart ? dayjs(omra.dateDepart) : null,
      });
  ...
  const filteredOmras = omras.filter((o) =>
    o.nom.toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  const columns = [
    ...
    { title: "Date de départ", dataIndex: "dateDepart", key: "dateDepart" },
